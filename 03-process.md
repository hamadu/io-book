...
プロセスは、ただ一つの親プロセスを持ち、複数の子プロセスを持つことができる。プロセスの親子関係は木構造を成している。親プロセスが子プロセスを持つには、**フォーク / fork** と呼ばれる自身のプロセスのコピーを作るシステムコールを発行する。コピーされた子プロセスは、親プロセスと同じプログラムを実行する。これを変更する場合は、`exec(2)` を使って自身のプロセスを他のプログラムで置き換える。

子プロセスは、親プロセスから以下の情報をコピーした上で引き継ぐ。

- 仮想アドレス空間をまるごと
- オープン中のファイルディスクリプタへの参照
- 環境変数

実際には fork した時点でデータがまるごとコピーされるのではなく、親か子のデータに変更があった場合に初めてコピーされる。この仕組みを **コピーオンライト　/ copy on write** と呼ぶ。このおかげで、fork 直後に exec した場合でも、fork の段階で余計なコピーコストがかかることはない。
...
子プロセスは親プロセスとは別物になるため、アドレス空間は共有しない。したがって、以下のプログラムは期待通りの動作をする。
...
実行中のプロセスを他のプログラムで置き換えたい場合、`exec` 関数群を使う。一例として、`execl` 関数を紹介する。
