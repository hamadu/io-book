...
スレッドとは、同じプログラムを見かけ上並行に実行する仕組みである。プロセスは複数のスレッドを持ちうる。プロセスは起動時に1つだけスレッドを持ち、このスレッドのことをメインスレッドと呼ぶ。同じプロセスに属するスレッドは、以下の内容を共有する。複数のスレッドで共有されている領域を触る場合、排他制御の仕組みが必要になる。
...
> ```
> [Man page of PTHREAD_JOIN](https://linuxjm.osdn.jp/html/glibc-linuxthreads/man3/pthread_join.3.html)

`th` には `pthread_create` で作成したスレッドを指定する。 `thread_return` を指定すると、スレッドで実行した関数の戻り値を格納することができる。


### 例: スレッドとヒープ・データ領域

`pthread_create`, `pthread_join` の使い方の確認を兼ねて、スレッド間で共有する情報とそうでないものを調べるため、いくつか実験をしよう。
...
このプログラムは別スレッドを立ち上げ `another` を実行させ、終了するのを待ってから変数の中身を確認する。`another` では、変数の操作をしてデータ出力し、再度変数の操作を行う。この操作がメインスレッドの出力内容に反映されるか確かめるのが目的だ。実行結果は次の通りで、別スレッドでの操作内容が反映されていることが分かる。理由は `global_num` がデータ領域に属していて、スレッド間で共有されるからだ。

```
hello from another thread: 200
hello from main thread: 300
```
...
Linuxにおけるスレッドの実装について軽く触れておく。内部的には、`clone` システムコールが使われている。`clone` とは、子プロセスを作るシステムコールである。`fork` と異なる点は、子プロセスで実行する関数を引数で指定できることと、親と何を共有するかを細かく指定できることである。

また、`fork` の内部の実装にも `clone` が使われている。カーネルの立場から見ると、スレッドとプロセスの違いはない。実際、プロセスとスレッドは同じ仕組みでスケジューリングされる。
...
`fn` には子プロセスで実行したい関数、`child_stack` には子プロセスが使うスタックの位置を指定する。 `flags` に親子間で共有する内容をビット論理和で指定する。例えば以下の値が利用可能である。

- CLONE_FILES: ファイルディスクリプタテーブルを共有する。
- CLONE_SIGHAND: シグナルハンドラテーブルを共有する。
- CLONE_THREAD: 子を親と同一のスレッドグループに置く。
- CLONE_VM: 仮想アドレス空間を共有する。


## 排他制御

複数のスレッドから同じデータ領域を触るとき、一連の操作のアトミック性を担保するには何かしらの排他制御の仕組みが必要になる。このうち、`mutex`, `read/write lock` と呼ばれる仕組みを紹介する。
...
mutex を使うには、mutex オブジェクトを初期化する必要がある。mutexオブジェクトを初期化するには `pthread_mutex_init` 関数を使うか、定数 `PTHREAD_MUTEX_INITIALIZER` を使う。今回は定数を使った使用例のみを示す。当然、これはグローバル変数として宣言しないと意味が無いので注意。
